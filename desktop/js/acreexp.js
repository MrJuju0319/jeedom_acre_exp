...
(function($) {
  ...
  window.addCmdToTable = function(_cmd) {
    ...
    tr.append('<td class="text-center"><span class="cmdId">' + init(_cmd.id, '-') + '</span><input type="hidden" class="cmdAttr" data-l1key="id" /></td>');
    tr.append('<td><input class="cmdAttr form-control input-sm" data-l1key="name" /></td>');
    tr.append('<td><span class="type">' + init(_cmd.type) + '</span><input type="hidden" class="cmdAttr" data-l1key="type" /></td>');
    tr.append('<td><span class="subType">' + init(_cmd.subType) + '</span><input type="hidden" class="cmdAttr" data-l1key="subType" /></td>');
    tr.append('<td><input class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="resource_path" /></td>');
    tr.append('<td><textarea class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="payload" rows="1"></textarea></td>');
    ...
    if (isset(_cmd.configuration.resource_path)) {
      tr.find('.cmdAttr[data-l2key=resource_path]').val(_cmd.configuration.resource_path);
    }

    if (isset(_cmd.configuration.payload)) {
      var payload = _cmd.configuration.payload;
      if (typeof payload === 'object') {
        payload = JSON.stringify(payload);
      }
      tr.find('.cmdAttr[data-l2key=payload]').val(payload);
    }
