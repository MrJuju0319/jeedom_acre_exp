...
class acreexp extends eqLogic {
    ...
    public static function deamon_info() {
        ...
        $watchdogActive = $watchdogInstalled && self::isWatchdogActive();

        $info = [
            'state' => ($daemonRunning || $watchdogActive) ? 'ok' : 'nok',
            'launchable' => 'ok',
            'launchable_message' => '',
        ];

        if (!$watchdogInstalled) {
            $info['launchable_message'] = __('Installez les dépendances pour déployer le watchdog système (optionnel).', __FILE__);
        } elseif (!$watchdogActive) {
            $info['launchable_message'] = __('Le service watchdog est inactif.', __FILE__);
        }
    ...
    public static function deamon_start($_debug = false) {
        ...
        if (self::isWatchdogInstalled()) {
            ...
            if ($code !== 0) {
                log::add('acreexp', 'warning', sprintf(__('Impossible de démarrer le service watchdog : %s', __FILE__), trim($output)));
            } else {
                $watchdogStarted = true;
                log::add('acreexp', 'info', __('Service watchdog démarré', __FILE__));
            }
        } else {
            log::add('acreexp', 'info', __('Watchdog système non installé, démarrage en mode PHP seul.', __FILE__));
        }
        ...
        log::add('acreexp', 'info', __('Boucle de rafraîchissement active', __FILE__));
    }
    ...
    public static function deamon_stop() {
        ...
        if (self::isWatchdogInstalled()) {
            ...
            log::add('acreexp', 'info', __('Watchdog système absent, seule la boucle PHP a été arrêtée.', __FILE__));
        }
    }
